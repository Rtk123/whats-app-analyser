...
def preprocess(data):
    # Regex patterns for matching date and time in different formats
    pattern = r'\d{2}\/\d{2}\/\d{2}, \d{1,2}:\d{2}\s(?:am|pm) - '
    regex1 = r'\d{2}/\d{2}/\d{4}, \d{1,2}:\d{2}\s*[ap]m - '
    regex2 = r'\d{1,2}\/\d{1,2}\/\d{2},\s\d{1,2}:\d{2}\s[AP]M\s-\s'

    # Extracting dates and messages
    dates1 = re.findall(regex1, data)
    dates2 = re.findall(pattern, data)
    dates3 = re.findall(regex2, data)

    messages1 = re.split(regex1, data)[1:]
    messages2 = re.split(pattern, data)[1:]
    messages3 = re.split(regex2, data)[1:]

    # Determine which regex worked based on the count of dates found
    holdsZero = [0, len(dates1), len(dates2), len(dates3)]
    dates = dates2
    messages = messages2
    if holdsZero[1]:
    ...
    # Replace special characters in dates
    dates = [date.replace('\u202f', ' ') for date in dates]
    ...
    # Splitting messages into user and message
    x[['user', 'message']] = x['messages'].str.split(': ', n=1, expand=True)
    ...
    x["month"] = pd.to_datetime(x["date"]).dt.month_name()
    x["day"] = pd.to_datetime(x["date"]).dt.day
    x['hour'] = pd.to_datetime(x["date"]).dt.hour
    x['minute'] = pd.to_datetime(x["date"]).dt.minute
    x['dayName'] = pd.to_datetime(x['date']).dt.day_name()
    x['monthName'] = pd.to_datetime(x['date']).dt.month_name()

    # Calculating periods and reply time
    period = [(f"{h}-{(h + 1) % 24}") for h in x['hour']]
    x['period'] = period
    x['replyTime'] = pd.to_datetime(x['date']).diff().fillna(pd.Timedelta(seconds=0))
    x = x[x['replyTime'] <= pd.Timedelta(days=2)]
